...
#
# undistort
dst_1_simp = cv2.undistort(img_1, mtx_1, dist_1)
# undistort
...
cv2.imwrite(os.path.join(outpath_1, troubleshoot_file_name_format[0] + "_original" + ".jpg"), img_1)
cv2.imwrite(os.path.join(outpath_1, troubleshoot_file_name_format[0] + "_undistorted" + ".jpg"), dst_1)
cv2.imwrite(os.path.join(outpath_1, troubleshoot_file_name_format[0] + "_undistorted_simp" + ".jpg"), dst_1_simp)

with open(os.path.join(outpath_1, 'camera_calibration.yaml'), 'w') as f:
    yaml.dump({'ret_1': ret_1,'newcameramtx_1': newcameramtx_1.tolist(),'mtx_1': mtx_1.tolist(), 'dist_1': dist_1.tolist(), 'fox_x': fov_x_1, 'fox_y': fov_y_1}, f)
